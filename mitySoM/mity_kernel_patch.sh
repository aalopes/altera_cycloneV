...
# Small script for configuring a vanilla kernel for the
# mitysom board. The drivers, etc. for the board are 
# available from at least 3.12 onwards.
# Note that after applying this patch, the kernel
# will not compile for several other boards!
#
# Alexandre Lopes
# 26.05.2015

MITYSOM_CONF=arch/arm/configs/mitysom5csx_devkit_defconfig
MITYSOM_DTS=arch/arm/boot/dts/socfpga_mitysom5csx_devkit.dts
SOCFPGA_DTS=arch/arm/boot/dts/socfpga.dtsi 
DTS_MAKEFILE=arch/arm/boot/dts/Makefile

cat << 'EOF' > $MITYSOM_CONF 
#
...
# CONFIG_NET_VENDOR_ALTERA is not set
...
# all devices other than the mitysom under the socfpga need
# to go. We will actually erase everything, so that this
# only compiles for the mitysom

cat << 'EOF' > $DTS_MAKEFILE
ifeq ($(CONFIG_OF),y)

dtb-$(CONFIG_ARCH_SOCFPGA) += socfpga_mitysom5csx_devkit.dtb
